...
(function () {
  ...
  function boot() {
    ...
    var RENDERERS = {
      ...
      /* ---- quiz: multiple-choice with instant feedback + running score ---- */
      quiz: function (p) {
        var qs = p.questions || [];
        var cards = qs.map(function (q, qi) {
          ...
          return '<div class="quiz-card" data-item data-answer="' + (q.answer || 0) + '">' +
            '<div class="quiz-q"><span class="quiz-num">' + (qi + 1) + "</span>" +
              "<span>" + esc(t(q.q)) + "</span></div>" +
            '<div class="quiz-opts">' + opts + "</div>" +
            '<div class="quiz-explain" hidden>' + esc(t(q.explain)) + "</div>" +
          "</div>";
        }).join("");
        return head(p) +
    ...
    var WIRE = {
      ...
      quiz: function () {
        var cards = [].slice.call(pageEl.querySelectorAll(".quiz-card"));
        var scoreEl = document.getElementById("quizScore");
        var total = cards.length, answered = 0, correct = 0;
        function paintScore() {
          if (!scoreEl) return;
          scoreEl.textContent = L.state.lang === "en"
            ? ("Score: " + correct + " / " + total + "  ·  answered " + answered)
            : ("得分：" + correct + " / " + total + "　·　已作答 " + answered);
        }
        paintScore();
        cards.forEach(function (card) {
          var ans = parseInt(card.dataset.answer, 10) || 0;
          ...
          opts.forEach(function (btn) {
            btn.addEventListener("click", function () {
              ...
              opts.forEach(function (b) {
                ...
                b.disabled = true;
                if (bi === ans) b.classList.add("quiz-opt--correct");
    ...
    /* ---- scroll-reveal: gentle fade-in for content blocks (editorial feel) ---- */
    function reveal() {
      var reduce = window.matchMedia && window.matchMedia("(prefers-reduced-motion: reduce)").matches;
      if (reduce || !("IntersectionObserver" in window)) return;
      /* scrolly steps run their own opacity logic — leave them alone */
      var els = [].slice.call(pageEl.querySelectorAll("[data-item]:not(.scrolly-step)"));
      if (!els.length) return;
